    ...
    IMapper mapper) : ControllerBase
{
    ...
    public async Task<IActionResult> DeleteAccount()
    {
        try
        {
            ...
            {
                return BadRequest(new { error = "Не вдалося отримати ім'я користувача." });
            }
            ...
            if (!result.Succeeded)
            {
                return BadRequest(new { error = "Не вдалося видалити акаунт." });
            }

            // Повертаємо успішний статус
            return Ok(new { message = "Акаунт успішно видалено." });
        }
        catch (Exception ex)
        {
